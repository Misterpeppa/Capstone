...
document.addEventListener("DOMContentLoaded", function () {
    var approve_appointment_tab_button = document.getElementById(
        "approve_appointment_tab_button",
    );
    var pending_appointment_tab_button = document.getElementById(
        "pending_appointment_tab_button",
    );
    var rejected_appointment_tab_button = document.getElementById(
        "rejected_appointment_tab_button",
    );

    var pending_appointment_btn = document.getElementById(
        "pending_appointment_btn",
    );
    var add_appointment_modal1 = new bootstrap.Modal(
        document.getElementById("add_appointment_modal"),
    );
    ...
    var submit_pending_appointment = document.getElementById(
        "submit_pending_appointment",
    );
    ...
    submit_pending_appointment.addEventListener("click", function () {
        ...
        var emptyState = document.getElementById(
            "pending_appointment_empty_state",
        );
    ...
    function handleAddPendingAppointmentBtnVisibility() {
        var addPendingAppointmentBtn = document.getElementById(
            "add_pending_appointment_split_btn",
        );
        var approvedAppointmentEmptyState = document.getElementById(
            "approved_appointment_empty_state",
        );
        var rejectedAppointmentEmptyState = document.getElementById(
            "rejected_appointment_empty_state",
        );
        var pendingAppointmentEmptyState = document.getElementById(
            "pending_appointment_empty_state",
        );

        // Check if approved, rejected, and pending tabs don't contain their respective empty states
        if (
            !approvedAppointmentEmptyState &&
            !rejectedAppointmentEmptyState &&
            !pendingAppointmentEmptyState
        ) {
    ...
    <defs>
        ...
        var rejectEmptyState = document.getElementById(
            "rejected_appointment_empty_state",
        );
        ...
        acceptAction.addEventListener("click", function (event) {
            ...
            var acceptModal = new bootstrap.Modal(
                document.getElementById("accept_modal"),
            );
            acceptModal.show();
            ...
            var approve_btn = document.getElementById(
                "confirm_approve_appointment_btn",
            );
        ...
        <defs>
            ...
            <rect width="24" height="24" fill="white"/>
                ...
                //mark complete modal that will appear after clicking completeAction
                var completeModal = new bootstrap.Modal(
                    document.getElementById("mark_complete_modal"),
                );
                ...
                //continue button for complete appointment
                var confirmCompleteAction =
                    document.querySelector(".confirm_complete");
                confirmCompleteAction.addEventListener(
                    "click",
        ...
        rejectAction.addEventListener("click", function (event) {
            ...
            var rejectModal = new bootstrap.Modal(
                document.getElementById("reject_modal"),
            );
            rejectModal.show();
            ...
            function moveToRejectedTable() {
                ...
                var patientInfoCell =
                    selectedRow.querySelector(":nth-child(4)");
                var dateTimeCell = selectedRow.querySelector(":nth-child(5)");
                var surgeryTypeCell =
                    selectedRow.querySelector(":nth-child(6)");

                patientInfoCell.innerHTML =
                    petNameInput1.value + " (" + breedInput1.value + ")" ||
                    "N/A";
                dateTimeCell.innerHTML =
                    (appointmentDateInput1.value || "N/A") +
                    "<br>" +
                    (appointmentTimeInput1.value || "N/A");
                surgeryTypeCell.innerHTML = surgeryTypeInput1.value || "N/A";

                var reasonInput = document.getElementById("rejection_reason");
                var specify_reasonInput =
                    document.getElementById("specify_reason");
        ...
        <defs>
            ...
            <rect width="24" height="24" fill="white"/>
                ...
                reschedAction.addEventListener("click", function () {
                    var reschedModal1 = new bootstrap.Modal(
                        document.getElementById("resched_modal-1"),
                    );
                    reschedModal1.show();
                });
                ...
                archiveAction.addEventListener("click", function () {
                    ...
                    var archiveModal = new bootstrap.Modal(
                        document.getElementById("archive_modal"),
                    );
                    archiveModal.show();
                });

                var rejectPagination = document.getElementById(
                    "rejected_pagination",
                );
        ...
        reschedAction.addEventListener("click", function () {
            var reschedModal = new bootstrap.Modal(
    ...
    function setupSubmitPendingButton() {
        ...
        function updateSubmitButtonState() {
            ...
            var isAppointmentDateValid =
                appointmentDateInput.value.trim() !== "";
            var isAppointmentTimeValid =
    ...
    var approvedEmptyState = document.getElementById(
        "approved_appointment_empty_state",
    );
    var pendingEmptyState = document.getElementById(
        "pending_appointment_empty_state",
    );
    var rejectedEmptyState = document.getElementById(
        "rejected_appointment_empty_state",
    );

    approveTab.addEventListener("click", function () {
        pendingBtn = document.getElementById(
            "add_pending_appointment_split_btn",
        );
...
function checkRadio(buttonId) {
    ...
    if (button) {
        ...
        if (radio) {
            ...
            radio.style.backgroundColor =
                "var(--Colors-Actions-action-active, #004D53)";
            radio.style.border =
                "1px solid var(--Colors-Actions-action-active, #004D53)";

            // Uncheck other radio buttons in the same group and reset their styles
            var otherRadios = document.querySelectorAll(
                "#" + buttonId + ' .checkbox[name="species"]',
            );
...
function setupEmailValidation() {
    ...
    function validateEmail() {
        ...
        if (
            enteredEmail.endsWith("@gmail.com") ||
            enteredEmail.endsWith("@yahoo.com")
        ) {
...
function setupRescheduleButton() {
    ...
    function updateRescheduleButtonState() {
        ...
        var isSpecifyReasonValid =
            specifyReasonInput.value.trim() !== "" ||
            rejectionReasonInput.value.trim() !== "other";
    ...
    rescheduleButton.addEventListener("click", function () {
        ...
        var successRescheduleModal = new bootstrap.Modal(
            document.getElementById("success_reschedule_modal"),
        );
...
function setupRescheduleButton1() {
    ...
    function updateRescheduleButtonState() {
        ...
        // Enable or disable the reschedule button based on the conditions
        rescheduleButton.disabled = !(
            isAppointmentDateValid && isAppointmentTimeValid
        );
    }
    // Close the resched_modal
    var reschedModal = new bootstrap.Modal(
